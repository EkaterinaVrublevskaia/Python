...
# вычисляющую элемент по номеру строки и столбца, т.е. функцию двух аргументов. Аргументы num_rows и num_columns 
...
def print_operation_table(operation, num_rows = 4, num_columns = 4):
    lst = [[operation(i, j) for i in range(1, num_rows + 1)] for j in range(1, num_columns + 1)]
    for i in lst:
        print(*[f"{el:<4}" for el in i])
